/*
 *  ExtendableAVFoundationVideoGrabber.mm
 */
...
@interface iOSExtendableVideoGrabber() <AVCaptureVideoDataOutputSampleBufferDelegate> {
...
@implementation iOSExtendableVideoGrabber
    ...
    if(grabberPtr != NULL) {
        @autoreleasepool {
            ...
            for(auto &delegate : delegates) {
                delegate->didOutputSampleBuffer(imageBuffer);
            }
...
ExtendableAVFoundationVideoGrabber::ExtendableAVFoundationVideoGrabber(){
    fps        = 30;
    grabber = [iOSExtendableVideoGrabber alloc];
    pixels    = NULL;
    width = 0;
    height = 0;
...
ExtendableAVFoundationVideoGrabber::~ExtendableAVFoundationVideoGrabber(){
    ofLog(OF_LOG_VERBOSE, "ExtendableAVFoundationVideoGrabber dtor");
    bLock = true;
    if(grabber) {
...
void ExtendableAVFoundationVideoGrabber::clear(){
    if( pixels != NULL ){
        free(pixels);
        pixels = NULL;
        free(pixelsTmp);
...
void ExtendableAVFoundationVideoGrabber::setCaptureRate(int capRate){
    fps = capRate;
}

bool ExtendableAVFoundationVideoGrabber::initGrabber(int w, int h){
...
bool ExtendableAVFoundationVideoGrabber::isInitialized(){
    return bIsInit;
}

void ExtendableAVFoundationVideoGrabber::update(){
    newFrame = false;
...
void ExtendableAVFoundationVideoGrabber::updatePixelsCB( CGImageRef & ref ){
...
bool ExtendableAVFoundationVideoGrabber::isFrameNew() {
    return newFrame;
}

vector <ofVideoDevice> ExtendableAVFoundationVideoGrabber::listDevices() {
...
void ExtendableAVFoundationVideoGrabber::setDevice(int deviceID) {
    [grabber setDevice:deviceID];
    device = deviceID;
}

bool ExtendableAVFoundationVideoGrabber::setPixelFormat(ofPixelFormat PixelFormat) {
...
ofPixelFormat ExtendableAVFoundationVideoGrabber::getPixelFormat() {
    if(internalGlDataType == GL_RGB){
...
CGImageRef ExtendableAVFoundationVideoGrabber::getCurrentFrame() const {
    return grabber.getCurrentFrame;
}

void ExtendableAVFoundationVideoGrabber::addDelegate(ofxiOSExtendableVideoGrabberDelegate *delegate)
{
    [grabber addDelegate:delegate];
}
void ExtendableAVFoundationVideoGrabber::removeDelegate(ofxiOSExtendableVideoGrabberDelegate *delegate)
{
    [grabber removeDelegate:delegate];
}
